...
function buildHollowbrookMaze() {
    ...
    const cells = buildBranchingMaze(17, 148, 1, 76, 3, 4, coreObstacles, edgeObstacles, 11001);

    // One shared, non-overlapping cell set for EVERYTHING placed in the
    // maze - 31 hostiles (the same total as the old three-segment pass:
    // 8+2 wolf/goblin, 8+2 boar/bear, 8+3 slime_water/bear/goblin) + 3
    // riddle-keepers + the hostage + the fox/deer pair + 8 loot items
    // (2 health potions, 2 general trinkets, and 4 originals including the
    // glowing_acorn key item) = 45 cells. Sampling without replacement
    // from one call guarantees distinct spots by construction, no matter
    // how many different beats share the same maze.
    const spots = sampleCells(cells, 45, 11002);
    let i = 0;

    if (!api.getVar("hollowbrook_hostiles_spawned", false)) {
        api.setVar("hollowbrook_hostiles_spawned", true);
        // Exactly the old three-segment totals, added up: wolf x5 (Segment
        // A x4 + Chamber A x1), goblin x6 (Segment A x4 + Chamber A x1 +
        // wood x1), boar x6 (Segment B x4 + Chamber B x1 + wood x1), bear
        // x8 (Segment B x4 + Segment C x4), slime_water x6 (Chamber B x1 +
        // Segment C x4 + wood x1) = 31.
        const hostileTypes = [
            "wolf", "wolf", "wolf", "wolf", "wolf",
            "goblin", "goblin", "goblin", "goblin", "goblin", "goblin",
            "boar", "boar", "boar", "boar", "boar", "boar",
            "bear", "bear", "bear", "bear", "bear", "bear", "bear", "bear",
            "slime_water", "slime_water", "slime_water", "slime_water", "slime_water", "slime_water",
        ];
