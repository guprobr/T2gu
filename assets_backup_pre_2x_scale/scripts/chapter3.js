...
function* onLevelStart() {
    ...
    yield api.wait(0.5);
    yield api.say("Juniper", "The roots stop here. Whatever's under them isn't ours to read anymore.");
...
function buildTheHollow() {
    ...
    const cells = buildBranchingMaze(30, 155, 1, 94, 3, 4, coreObstacles, edgeObstacles, 31001);

    // One shared, non-overlapping cell set (32 hostiles + 3 riddle-keepers
    // + the hostage + 7 loot items - 2 health potions, 2 general
    // trinkets, and 3 originals = 43 cells) - sampling without
    // replacement from a single call guarantees distinct spots, no matter
    // how dense.
    const spots = sampleCells(cells, 43, 31002);
    if (!api.getVar("hollow_hostiles_spawned", false)) {
        ...
        // troll x1, mummy x1, skeleton_swordsman x1), repeated 4x - "for
    ...
    // (remembers).
    api.spawnNpc("gnome_alchemist", spots[32].col, spots[32].row);
    api.spawnNpc("earth_spirit", spots[33].col, spots[33].row);
    api.spawnNpc("tribal_elder_man", spots[34].col, spots[34].row);

    if (!api.getVar("hostage_spawned", false)) {
        api.setVar("hostage_spawned", true);
        api.spawnNpc("miner", spots[35].col, spots[35].row);
    }

    if (api.getVar("hollow_loot_spawned", false))
        return;
    api.setVar("hollow_loot_spawned", true);
    api.spawnItem("ore_chunk", spots[36].col, spots[36].row);
    api.spawnItem("iron_sword", spots[37].col, spots[37].row);
    api.spawnItem("antidote_vial", spots[38].col, spots[38].row);
    api.spawnItem("health_potion", spots[39].col, spots[39].row);
    api.spawnItem("health_potion", spots[40].col, spots[40].row);
    api.spawnItem("repair_kit", spots[41].col, spots[41].row);
    api.spawnItem("gemstone_cluster", spots[42].col, spots[42].row);
}
...
function* talkToCobb() {
    ...
    if (timesTalked === 0) {
        ...
        yield api.say("Juniper", "Why hasn't anyone gone in, if it's that close?");
...
function* onItemCollected(itemId) {
    ...
    yield api.say("Juniper", "You could just answer her directly, you know.");
